...
- [x] CPU perf: Chunk/metadata work still ~6 ms/frame at ~1.1–1.7k loaded chunks (profile.csv). Amortize chunk metadata refresh across frames (e.g., slice X/Z ring per frame) to cap per-frame time. _Done: metadata_state slices 256 chunk slots per frame, seeds GPU buffers on origin shifts, and reuses persistent CPU caches._
