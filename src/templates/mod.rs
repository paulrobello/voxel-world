...
impl TemplateLibrary {
    ...
    /// Sanitizes a name for use as a filename (alphanumeric, underscore, hyphen only).
    ...
    pub fn save_template(&self, template: &VxtFile) -> io::Result<()> {
        ...
        let safe_name = Self::sanitize_name(&template.name);
    ...
    pub fn delete_template(&self, name: &str) -> io::Result<()> {
        ...
        if path.exists() {
            ...
            let thumbnail_path = self.get_thumbnail_path(&safe_name);
