...
pub fn create_initial_world_with_seed(
    ...
    if !chunks_to_generate.is_empty() {
        ...
        let results: Vec<(Vector3<i32>, ChunkGenerationResult)> = chunks_to_generate
            ...
            .map(|&chunk_pos| {
                ...
                if count.is_multiple_of(progress_interval) || count == chunks_to_gen_count {
        ...
        log::debug!(""); // New line after progress
    ...
    if elapsed.as_secs_f32() > 0.0 {
        ...
        log::debug!("[World Gen] Throughput: {:.0} chunks/sec", chunks_per_sec);
    }
