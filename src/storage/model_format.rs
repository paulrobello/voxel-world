...
impl VxmFile {
    ...
    pub fn from_model(model: &SubVoxelModel, author: String) -> Self {
        ...
        Self {
            ...
            palette_emission: model.palette_emission_slice().to_vec(),
    ...
    pub fn to_model(&self) -> SubVoxelModel {
        ...
        // Load palette emission values, collapsing to the sparse `None` form if all zero.
        let mut palette_emission_arr = [0.0f32; PALETTE_SIZE];
        for (i, &emission) in self.palette_emission.iter().take(PALETTE_SIZE).enumerate() {
            palette_emission_arr[i] = emission;
        }
        let palette_emission: Option<Box<[f32; PALETTE_SIZE]>> =
            if palette_emission_arr.iter().any(|&e| e > 0.0) {
                Some(Box::new(palette_emission_arr))
            } else {
                None
            };
