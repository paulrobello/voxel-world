...
impl Player {
    ...
    pub fn check_collision(
        ...
        self.check_collision_ex(feet_pos, false, world, model_registry, collision_enabled)
    }
                ...
                for bz in min_z..=max_z {
                    ...
                    if let Some(block_type) = world.get_block(world_pos) {
                        if block_type.is_solid() {
                            ...
                            if skip_ladders
                                && let Some(model_data) = world.get_model_data(world_pos)
                                && ModelRegistry::is_ladder_model(model_data.model_id)
                            {
                                continue;
                            }
            ...
            for by in min_y..=max_y {
                for bz in min_z..=max_z {
                    ...
                    if let Some(BlockType::Model) = world.get_block(pos)
                        && let Some(model_data) = world.get_model_data(pos)
                        && ModelRegistry::is_ladder_model(model_data.model_id)
                    {
                        return true;
                    }
