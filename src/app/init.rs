...
use super::App;
use crate::app_state::{
    AutoProfileFeature, FrameState, Graphics, HotbarState, InputState, MinimapUiState,
    MultiplayerState, PaletteUiState, PictureUiState, PlacementState, ProfileState, StreamingState,
    UiState, WorldSim,
};
...
impl App {
    pub fn new(event_loop: &EventLoop<()>) -> Self {
        ...
        if let Some(ref data_dir) = args.data_dir {
            ...
            set_data_dir(&path);
            log::debug!("[Launcher] Using data directory: {}", path.display());
        }
        ...
        log::debug!("[Launcher] Loading world: '{}'", world_name);
        ...
        if PathBuf::from("world").exists() && !world_dir.exists() && world_name == "default" {
            log::debug!("[Launcher] Migrating legacy world to 'worlds/default'...");
        ...
        let mut view_distance = args.view_distance.unwrap_or(VIEW_DISTANCE);
        let mut load_distance = LOAD_DISTANCE;
        let mut unload_distance = UNLOAD_DISTANCE;
        let mut show_minimap = prefs.show_minimap;
        let mut player_light_enabled = true;
        let mut atmosphere = atmosphere::AtmosphereSettings::default();
        let mut settings = prefs.settings.clone();

        if let Some(preset) = args.quality {
        ...
        let atlas = load_texture_atlases(
            vk.memory_allocator.clone(),
            vk.command_buffer_allocator.clone(),
            vk.descriptor_set_allocator.clone(),
            &render_pipeline,
        ...
        let texture_set = atlas.descriptor_set;
        let texture_atlas_view = atlas.main_image_view;
        let custom_texture_atlas = atlas.custom_image;
        let picture_atlas_view = atlas.picture_image_view;
        let picture_atlas_image = atlas.picture_image;

        // Create particle, falling block, water source, template block, and stencil block buffers (share set 3)
        let sim_buffers = get_particle_and_falling_block_set(
            vk.memory_allocator.clone(),
            vk.descriptor_set_allocator.clone(),
            &render_pipeline,
        );
        let particle_buffer = sim_buffers.particle_buffer;
        let falling_block_buffer = sim_buffers.falling_block_buffer;
        let water_source_buffer = sim_buffers.water_source_buffer;
        let template_block_buffer = sim_buffers.template_block_buffer;
        let stencil_block_buffer = sim_buffers.stencil_block_buffer;
        let remote_player_buffer = sim_buffers.remote_player_buffer;
        let particle_set = sim_buffers.descriptor_set;
        ...
        match model_registry.load_library_models(&library_path) {
            ...
            Err(e) => {
                log::warn!("Warning: Failed to load library models: {}", e);
            }
            _ => {}
        }

        // Create combined brick metadata and model resources (set 7)
        let bm = get_brick_and_model_set(
            vk.memory_allocator.clone(),
            vk.command_buffer_allocator.clone(),
            vk.descriptor_set_allocator.clone(),
            &render_pipeline,
        ...
        let brick_mask_buffer = bm.brick_mask_buffer;
        let brick_dist_buffer = bm.brick_dist_buffer;
        let model_atlas_8 = bm.model_atlas_8;
        let model_atlas_16 = bm.model_atlas_16;
        let model_atlas_32 = bm.model_atlas_32;
        let model_palettes = bm.model_palettes;
        let model_palette_emission = bm.model_palette_emission;
        let model_metadata = bm.model_metadata;
        let block_custom_data = bm.block_custom_data;
        let model_properties_buffer = bm.model_properties_buffer;
        let brick_and_model_set = bm.descriptor_set;
        ...
        player.light_enabled = player_light_enabled;
        ...
        let graphics = Graphics {
            ...
            device: vk.device,
            queue: vk.queue,
            transfer_queue: vk.transfer_queue,
            graphics_queue_family: vk.graphics_queue_family,
            transfer_queue_family: vk.transfer_queue_family,
            separate_transfer_queue: vk.separate_transfer_queue,
            ...
            multiplayer_texture_array: None,
            multiplayer_texture_array_view: None,
            multiplayer_texture_sampler: None,
            multiplayer_texture_count: 0,
            particle_buffer,
            particle_set,
            light_buffer,
            light_set,
        ...
        let profile_log_path = if args.profile || args.auto_profile {
        ...
        let ui = UiState {
            settings: {
                ...
                if args.show_chunk_boundaries {
                    settings.show_chunk_boundaries = true;
                }
                settings
            },
            hotbar: HotbarState {
            ...
            palette_ui: PaletteUiState::default(),
            ...
            template_library: {
                ...
                if let Err(e) = lib.init() {
                    log::warn!("Failed to initialize template library: {}", e);
                }
                lib
            },
            stencil_library: {
                ...
                if let Err(e) = lib.init() {
                    log::warn!("Failed to initialize stencil library: {}", e);
                }
                lib
            },
            stencil_manager,
        ...
        if args.is_multiplayer() {
            ...
            match args.game_mode() {
                crate::config::GameMode::Host => {
                    log::debug!("[Multiplayer] Starting server on port {}...", args.port);
                    match multiplayer.start_host(
                        ...
                        Err(e) => log::warn!("[Multiplayer] Failed to start server: {}", e),
                    }
                }
