...
pub fn print_stats(ui: &mut UiState, sim: &mut WorldSim, verbose: bool) {
    ...
    let frame_time_ms = if ui.frame.fps > 0 {
        1000.0 / ui.frame.fps as f32
    } else {
        0.0
    };

    let render_res = [
        (ui.frame.window_size[0] as f32 * ui.settings.render_scale) as u32,
        (ui.frame.window_size[1] as f32 * ui.settings.render_scale) as u32,
    ];
    if verbose {
        log::debug!(
            ...
            ui.frame.fps,
            frame_time_ms,
            ui.frame.window_size[0],
            ui.frame.window_size[1],
            render_res[0],
            render_res[1],
    ...
    if let Some(path) = &ui.profile.profile_log_path {
        ...
        if let Ok(mut file) = file_result {
            ...
            let _ = writeln!(
                ...
                ui.minimap_ui.show_minimap as u8,
                ui.minimap_ui.minimap.size,
                ui.minimap_ui.minimap.skip_decorative as u8,
