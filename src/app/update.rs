...
impl App {
    ...
    pub fn update(&mut self, event_loop: &ActiveEventLoop) {
        // Wait for the previous frame's GPU work to finish before writing GPU buffers.
        // This enables CPU/GPU pipelining: render() stores the fence instead of waiting,
        // so CPU work between render() and the next update() overlaps with GPU execution.
        // Dropping the boxed future triggers FenceSignalFuture's Drop impl which waits.
        drop(self.previous_frame_fence.take());

        self.ui.frame.total_frames += 1;
        let now = Instant::now();

        // Check for screenshot delay
        if let Some(delay) = self.args.screenshot_delay
            && !self.ui.frame.screenshot_taken
        {
        ...
        if let Some(delay) = self.args.exit_delay {
            let elapsed = now.duration_since(self.start_time).as_secs_f64();
            if elapsed >= delay {
                log::debug!("[EXIT] Exiting after {:.1}s delay", elapsed);
                self.sim.save_all(
        ...
        if let Some(duration) = self.args.benchmark_duration {
            let elapsed = now.duration_since(self.start_time).as_secs_f64();
            if elapsed >= duration {
                log::debug!("[BENCHMARK] Complete after {:.1}s", duration);
                self.sim.save_all(
                    &self.ui.placement.measurement_markers,
                    &self.ui.stencil_manager,
                );
        ...
        if now.duration_since(self.ui.frame.last_second) > Duration::from_secs(1) {
            self.ui.frame.fps = self.ui.frame.frames_since_last_second;
            self.ui.frame.frames_since_last_second = 0;
            self.ui.frame.last_second = now;

            // Update smoothed FPS (exponential moving average, alpha=0.15 for stability)
            let alpha = 0.15;
            self.ui.frame.smoothed_fps =
                self.ui.frame.smoothed_fps * (1.0 - alpha) + self.ui.frame.fps as f32 * alpha;
            ...
            if self.ui.settings.dynamic_render_scale {
                ...
                let fps_error = self.ui.frame.smoothed_fps - target;
                ...
                if fps_error.abs() > deadband {
                    ...
                    if (new_scale - current_scale).abs() > 0.001 {
                        ...
                        self.ui.frame.pending_scale_change = true;
                    }
                }
            }
            ...
            if self.ui.profile.auto_profile_enabled {
                // Phase duration is dynamic: 5s for toggle tests, 30s for flying
                let phase_duration =
                    Duration::from_secs(self.ui.profile.auto_profile_feature.duration_secs());
                let phase_elapsed = now.duration_since(self.ui.profile.auto_profile_phase_start);

                if phase_elapsed >= phase_duration {
                    self.ui.profile.auto_profile_phase_start = now;

                    match self.ui.profile.auto_profile_feature {
                        AutoProfileFeature::Baseline => {
                            // Move to first feature test (AO OFF)
                            self.ui.profile.auto_profile_feature = AutoProfileFeature::AO;
                            self.ui.profile.auto_profile_feature_off = true;
                            self.ui.settings.enable_ao = false;
                            log::debug!("[AUTO-PROFILE] Testing AO: OFF");
                        }
                        AutoProfileFeature::Flying => {
                            // Flying phase complete, move to Done
                            self.ui.profile.auto_profile_feature = AutoProfileFeature::Done;
                            self.sim.player.auto_fly_enabled = false;
                            log::debug!("[AUTO-PROFILE] Flying phase complete.");
                        }
                        AutoProfileFeature::Done => {
                            // Exit the application
                            log::debug!("[AUTO-PROFILE] Complete! Exiting...");
                            self.sim.save_all(
                                &self.ui.placement.measurement_markers,
                                &self.ui.stencil_manager,
                            );
                        ...
                        _ => {
                            if self.ui.profile.auto_profile_feature_off {
                                // Feature was OFF/MIN, turn it back ON/MAX
                                self.ui.profile.auto_profile_feature_off = false;
                                match self.ui.profile.auto_profile_feature {
                                    ...
                                    AutoProfileFeature::Minimap => {
                                        self.ui.minimap_ui.show_minimap = true
                                    }
                                    AutoProfileFeature::MinimapSkipDecorative => {
                                        self.ui.minimap_ui.minimap.skip_decorative = true;
                                        self.ui.minimap_ui.minimap_cached_image = None; // Clear cache
                                ...
                                let state_name = match self.ui.profile.auto_profile_feature {
                                ...
                                log::debug!(
                                    ...
                                    self.ui.profile.auto_profile_feature.name(),
                                    state_name
                                );
                            } else {
                                // Feature was ON/MAX, move to next feature (OFF/MIN or Flying)
                                self.ui.profile.auto_profile_feature =
                                    self.ui.profile.auto_profile_feature.next();
                                self.ui.profile.auto_profile_feature_off = true;
                                match self.ui.profile.auto_profile_feature {
                                    ...
                                    AutoProfileFeature::Minimap => {
                                        self.ui.minimap_ui.show_minimap = false
                                    }
                                    AutoProfileFeature::MinimapSkipDecorative => {
                                        self.ui.minimap_ui.minimap.skip_decorative = false;
                                        self.ui.minimap_ui.minimap_cached_image = None; // Clear cache
                                        self.sim.world.clear_minimap_cache();
                                    }
                                    AutoProfileFeature::HideGroundCover => {
                                    ...
                                    AutoProfileFeature::Flying => {
                                        ...
                                        log::debug!(
                                            ...
                                            self.ui.profile.auto_profile_feature.duration_secs()
                                        );
                                        self.ui.profile.auto_profile_feature_off = false; // No OFF phase
                                    }
                                    AutoProfileFeature::Done => {
                                ...
                                if self.ui.profile.auto_profile_feature
                                    != AutoProfileFeature::Flying
                                    && self.ui.profile.auto_profile_feature
                                        != AutoProfileFeature::Done
                                {
        ...
        self.ui.frame.frames_since_last_second += 1;
        ...
        if self.args.debug_interval > 0
            && self
                ...
                .frame
                .total_frames
                .is_multiple_of(self.args.debug_interval as u64)
        {
            let player_pos = self
                .sim
                .player
        ...
        if loaded > 0 {
            self.ui.minimap_ui.minimap_cached_image = None;
        }
        ...
        if self.sim.model_registry.is_gpu_dirty() {
            upload_model_registry(
                ...
                ModelAtlasTargets {
                    atlas_8: &self.graphics.model_atlas_8,
                    atlas_16: &self.graphics.model_atlas_16,
                    atlas_32: &self.graphics.model_atlas_32,
                    palettes: &self.graphics.model_palettes,
                    palette_emission: &self.graphics.model_palette_emission,
                    properties_buffer: &self.graphics.model_properties_buffer,
                },
            );
        ...
        if self.multiplayer.mode != crate::config::GameMode::SinglePlayer {
            ...
            if self.multiplayer.mode == crate::config::GameMode::Host {
                ...
                let player_velocity = self.sim.player.velocity;
                ...
                self.multiplayer.update_host_position(
                    ...
                    [
                        player_velocity.x as f32,
                        player_velocity.y as f32,
                        player_velocity.z as f32,
                    ],
                    player_yaw,
                    player_pitch,
                );
            ...
            if self.multiplayer.has_pending_server_seed()
                && let Some((seed, world_gen_byte)) = self.multiplayer.take_pending_server_seed()
            {
                ...
                || self.multiplayer.mode == crate::config::GameMode::Host
            {
                ...
                if self.multiplayer.has_pending_chunks() {
                    ...
                    for (pos, chunk) in chunks {
                        ...
                        log::debug!("[Client] Applied received chunk at {:?}", pos);
                    }
                }
            }
            ...
            if self.multiplayer.is_hosting() {
                ...
                // Frame-distributed BulkOperation application (H13) — drains a
                // 1000-block chunk of pending Fill/Replace work per tick and
                // broadcasts the resulting BlocksChanged to connected peers.
                self.fulfill_bulk_ops();
            ...
            // Process template loads received from server (client-side)
            self.register_pending_templates();

            // Process template removals received from server (client-side)
            self.remove_pending_templates();
        }

        if self.input.close_requested() {
            log::debug!("[Storage] Saving world before exit...");
            self.sim.save_all(
                &self.ui.placement.measurement_markers,
                &self.ui.stencil_manager,
            );
            log::debug!("[Prefs] Saving user preferences...");
            self.save_preferences();
            event_loop.exit();
            return;
        }
        ...
        if !self.ui.palette_ui.palette_open
            && self.ui.palette_ui.palette_previously_focused
            && !self.input.focused
            && !other_panel_open
        {
        ...
        // Update multiplayer chat display timer
        if self.multiplayer.mode != crate::config::GameMode::SinglePlayer {
            self.multiplayer.update_chat_timer(delta_time as f32);

            // Process pending chat messages from server
            if self.multiplayer.has_pending_chat_messages() {
                for chat in self.multiplayer.take_pending_chat_messages() {
                    self.multiplayer
                        .add_chat_message(chat.player_name, chat.message);
                }
            }

            // Process pending player name changes from server (show in console)
            if self.multiplayer.has_pending_player_name_changes() {
                for change in self.multiplayer.take_pending_player_name_changes() {
        ...
        if !self.multiplayer.is_client() {
            let player_pos_f32 = self
                ...
                .feet_pos(self.sim.world_extent, self.sim.texture_origin)
                .cast::<f32>();
            let (spawn_events, model_break_events) = self.sim.block_updates.process_updates(
                &mut self.sim.world,
            ...
            if self.multiplayer.is_host() && !spawn_events.is_empty() {
                for event in spawn_events {
                    ...
                    self.multiplayer.broadcast_falling_block_spawn(
                        event.entity_id,
                        position,
                        event.block_type,
                    );
            ...
            // Broadcast model ground support breaks to all clients when hosting
            // These are torches, fences, gates, etc. that broke because they lost support
            if self.multiplayer.is_host() && !model_break_events.is_empty() {
                use crate::net::protocol::{BlockChanged, BlockData};
                for event in model_break_events {
                    let change = BlockChanged {
                        position: [event.position.x, event.position.y, event.position.z],
                        block: BlockData::from(crate::chunk::BlockType::Air),
                    };
