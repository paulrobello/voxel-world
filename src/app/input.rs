...
use crate::constants::ATLAS_TILE_COUNT_U8;
...
impl App {
    ...
    fn next_paint_texture(&self, current: u8, step: i8) -> u8 {
        let mut indices: Vec<u8> = (0..ATLAS_TILE_COUNT_U8).collect();
    ...
    /// Returns true if any cursor-needing panel is currently open.
    fn any_panel_open(&self) -> bool {
        self.ui.palette_ui.palette_open
    ...
    /// Restores game focus if no cursor-needing panels remain open and `previously_focused` is set.
    ///
    /// Returns `true` if focus was restored. Call this after closing a panel, then clear
    /// the `previously_focused` flag when the return value is `true`.
    fn restore_focus_if_alone(&mut self, previously_focused: bool) -> bool {
        if !self.any_panel_open() && previously_focused {
    ...
    pub fn handle_focus_toggles(&mut self) -> bool {
        ...
        if self.input.key_pressed(KeyCode::Escape) && self.ui.palette_ui.palette_open {
            self.ui.palette_ui.palette_open = false;
            self.ui.palette_ui.dragging_item = None;
            if self.restore_focus_if_alone(self.ui.palette_ui.palette_previously_focused) {
        ...
        if self.input.key_pressed(KeyCode::Escape) && self.ui.multiplayer_panel.open {
            ...
            if self.restore_focus_if_alone(self.ui.multiplayer_panel.previously_focused) {
        ...
        if self.input.key_pressed(KeyCode::Escape) && self.ui.editor.active {
            ...
            if self.restore_focus_if_alone(self.ui.editor_previously_focused) {
                self.ui.editor_previously_focused = false;
            }
            log::debug!("Model editor: OFF");
            return true;
        }
        ...
        if self.input.key_pressed(KeyCode::Escape) && self.ui.console.active {
            ...
            if self.restore_focus_if_alone(self.ui.console_previously_focused) {
        ...
        if self.input.key_pressed(KeyCode::Escape) && self.ui.active_placement.is_some() {
            if let Some(ref placement) = self.ui.active_placement {
                log::debug!("Cancelled template placement: {}", placement.template.name);
            }
        ...
        if self.input.key_pressed(KeyCode::Escape) && self.ui.active_stencil_placement.is_some() {
            if let Some(ref placement) = self.ui.active_stencil_placement {
                log::debug!("Cancelled stencil placement: {}", placement.stencil.name);
            }
        ...
        if self.input.key_pressed(KeyCode::Escape) && self.ui.placement.flood_fill_active {
            log::debug!("Flood Fill Mode: OFF");
            self.ui.placement.flood_fill_active = false;
            return true;
        }

        // Cancel sphere tool
        if self.input.key_pressed(KeyCode::Escape) && self.ui.sphere_tool.active {
            log::debug!("Sphere Tool: OFF");
        ...
        if self.input.key_pressed(KeyCode::Escape) && self.input.focused {
            ...
            log::debug!("Unfocused - cursor will be released");
        }
        ...
        let panel_open = self.ui.palette_ui.palette_open
            || self.ui.editor.active
            || self.ui.console.active
    ...
    pub fn handle_focused_controls(&mut self, delta_time: f64) {
        ...
        if ds.1.abs() > 0.1 {
            self.ui.hotbar.hotbar_index = if ds.1 > 0.0 {
                (self.ui.hotbar.hotbar_index + self.ui.hotbar.hotbar_blocks.len() - 1)
                    % self.ui.hotbar.hotbar_blocks.len()
            } else {
                (self.ui.hotbar.hotbar_index + 1) % self.ui.hotbar.hotbar_blocks.len()
            };
        }

        // Number keys 1-9 to select hotbar slot
        if self.input.key_pressed(KeyCode::Digit1) {
            self.ui.hotbar.hotbar_index = 0;
        }
        if self.input.key_pressed(KeyCode::Digit2) {
            self.ui.hotbar.hotbar_index = 1;
        }
        if self.input.key_pressed(KeyCode::Digit3) {
            self.ui.hotbar.hotbar_index = 2;
        }
        if self.input.key_pressed(KeyCode::Digit4) {
            self.ui.hotbar.hotbar_index = 3;
        }
        if self.input.key_pressed(KeyCode::Digit5) {
            self.ui.hotbar.hotbar_index = 4;
        }
        if self.input.key_pressed(KeyCode::Digit6) {
            self.ui.hotbar.hotbar_index = 5;
        }
        if self.input.key_pressed(KeyCode::Digit7) {
            self.ui.hotbar.hotbar_index = 6;
        }
        if self.input.key_pressed(KeyCode::Digit8) {
            self.ui.hotbar.hotbar_index = 7;
        }
        if self.input.key_pressed(KeyCode::Digit9) {
            self.ui.hotbar.hotbar_index = 8;
        }
        ...
        if self.input.key_pressed(KeyCode::KeyF) {
            ...
            log::debug!("Fly mode: {}", if new_mode { "ON" } else { "OFF" });
        }
        ...
        if self.input.key_pressed(KeyCode::KeyM) {
            self.ui.minimap_ui.show_minimap = !self.ui.minimap_ui.show_minimap;
            log::debug!(
                "Minimap: {}",
                if self.ui.minimap_ui.show_minimap {
                    "ON"
                } else {
                    "OFF"
                }
        ...
        if self.input.key_pressed(KeyCode::KeyG) {
            self.ui.placement.rangefinder_active = !self.ui.placement.rangefinder_active;
            log::debug!(
                "Rangefinder: {}",
                if self.ui.placement.rangefinder_active {
                    "ON"
                } else {
                    "OFF"
                }
        ...
        if self.input.key_pressed(KeyCode::KeyK) {
            ...
            if self.ui.settings.show_water_sources {
                ...
                log::debug!("Water/Lava sources: OFF");
            }
        }
    ...
    pub fn handle_block_interactions(&mut self, delta_time: f32) {
            ...
            if let Some(hit) = self.ui.placement.current_hit {
                ...
                if self.input.mouse_pressed(MouseButton::Left) {
                    ...
                    if let Some((min, max)) = self.ui.template_selection.bounds()
                        && let Some((w, h, d)) = self.ui.template_selection.dimensions()
                    {
                        log::debug!(
                            "Selection: {}×{}×{} (from ({},{},{}) to ({},{},{}))",
                            w,
            ...
            && !self.ui.template_selection.visual_mode
        {
            ...
            if self.input.mouse_pressed(MouseButton::Left)
                && let Some(hit) = self.ui.placement.current_hit
            {
                // Use the actual block position (not placement position)
                let pos = hit.block_pos;

                // Check if marker already exists at this position
                if !self.ui.placement.measurement_markers.contains(&pos) {
                    if self.ui.placement.measurement_markers.len() < 4 {
                        self.ui.placement.measurement_markers.push(pos);
                        log::debug!(
                            "Measurement marker {} placed at ({}, {}, {})",
                            self.ui.placement.measurement_markers.len(),
                            pos.x,
                            pos.y,
                            pos.z
                        );
                        ...
                        if self.ui.placement.measurement_markers.len() >= 2 {
                            let prev = self.ui.placement.measurement_markers
                                [self.ui.placement.measurement_markers.len() - 2];
                            ...
                            log::debug!(
                                ...
                                self.ui.placement.measurement_markers.len() - 1,
                                dist
                            );
                        }
                    } else {
            ...
            if self.input.mouse_pressed(MouseButton::Right) {
                if let Some(removed) = self.ui.placement.measurement_markers.pop() {
                    log::debug!(
        ...
        if self.ui.placement.place_needs_reclick && !self.input.mouse_held(MouseButton::Right) {
            self.ui.placement.place_needs_reclick = false;
        }
            ...
            && let Some(hit) = self.ui.placement.current_hit
        {
            ...
            if self.ui.bridge_tool.start_position.is_some() {
                ...
                log::debug!("Bridge start: ({}, {}, {})", target.x, target.y, target.z);
            }
            self.ui.placement.place_needs_reclick = true;
            return; // Skip block placement
        }
            ...
            && let Some(hit) = self.ui.placement.current_hit
        {
            ...
            if self.ui.wall_tool.start_position.is_some() {
                ...
                log::debug!("Wall start: ({}, {}, {})", target.x, target.y, target.z);
            }
            self.ui.placement.place_needs_reclick = true;
            return; // Skip block placement
        }
            ...
            && let Some(hit) = self.ui.placement.current_hit
        {
            ...
            if self.ui.floor_tool.start_position.is_some() {
                ...
                log::debug!("Floor start: ({}, {}, {})", target.x, target.y, target.z);
            }
            self.ui.placement.place_needs_reclick = true;
            return; // Skip block placement
        }
            ...
            && let Some(hit) = self.ui.placement.current_hit
        {
            ...
            if self.ui.stairs_tool.start_pos.is_some() {
                ...
                log::debug!("Stairs start: ({}, {}, {})", target.x, target.y, target.z);
            }
            self.ui.placement.place_needs_reclick = true;
            return; // Skip block placement
        }
        ...
        if self.input.focused && self.ui.mirror_tool.active && self.input.key_pressed(KeyCode::Tab)
        {
            ...
            log::debug!("Mirror axis: {}", self.ui.mirror_tool.axis.name());
        }
        ...
        {
            if let Some(hit) = self.ui.placement.current_hit {
                let target_block = self.ui.hotbar.hotbar_blocks[self.ui.hotbar.hotbar_index];
                ...
                match crate::console::commands::floodfill(
                    ...
                    crate::console::CommandResult::Error(msg) => {
                        log::debug!("Flood fill error: {}", msg);
                    }
                    _ => {}
                }
            }
        ...
        if self.input.focused && self.input.mouse_pressed(MouseButton::Left) {
            ...
            if self.ui.bridge_tool.active && self.ui.bridge_tool.start_position.is_some() {
                ...
                log::debug!("Bridge placement cancelled");
                self.ui.placement.skip_break_until_release = true;
                return;
            }
            ...
            if self.ui.wall_tool.active && self.ui.wall_tool.start_position.is_some() {
                ...
                log::debug!("Wall placement cancelled");
                self.ui.placement.skip_break_until_release = true;
                return;
            }
            ...
            if self.ui.floor_tool.active && self.ui.floor_tool.start_position.is_some() {
                ...
                log::debug!("Floor placement cancelled");
                self.ui.placement.skip_break_until_release = true;
                return;
            }
            ...
            if self.ui.stairs_tool.active && self.ui.stairs_tool.start_pos.is_some() {
                ...
                log::debug!("Stairs placement cancelled");
                self.ui.placement.skip_break_until_release = true;
                return;
            }
                ...
                && self.ui.arch_tool.start_position.is_some()
            {
                ...
                log::debug!("Arch placement cancelled");
                self.ui.placement.skip_break_until_release = true;
                return;
            }
            ...
            if self.ui.pattern_fill.active {
                log::debug!("Pattern Fill Tool: OFF");
            ...
            if self.ui.scatter_tool.active {
                log::debug!("Scatter Brush Tool: OFF");
            ...
            && self.ui.active_stencil_placement.is_none()
        {
            ...
            if self.ui.placement.skip_break_until_release && !holding_break {
                self.ui.placement.skip_break_until_release = false;
            }

            // Skip block breaking until mouse is released after focusing
            if !self.ui.placement.skip_break_until_release {
        ...
        if self.input.focused
            && self.input.mouse_pressed(MouseButton::Middle)
            && let Some(hit) = self.ui.placement.current_hit
            && let Some(block_type) = self.sim.world.get_block(hit.block_pos)
            && block_type != BlockType::Air
        {
            ...
            if block_type == BlockType::Model {
                ...
                && let Some(data) = self.sim.world.get_paint_data(hit.block_pos)
            {
            ...
            {
                // Switch to that slot
                self.ui.hotbar.hotbar_index = idx;
                log::debug!("Picked {:?} (slot {})", block_type, idx + 1);
            } else {
                // Replace current slot with the picked block
                self.ui.hotbar.hotbar_blocks[self.ui.hotbar.hotbar_index] = block_type;
                log::debug!(
                    "Replaced slot {} with {:?}",
                    self.ui.hotbar.hotbar_index + 1,
                    block_type
            ...
            let idx = self.ui.hotbar.hotbar_index;
            match block_type {
                BlockType::Model => {
                    self.ui.hotbar.hotbar_model_ids[idx] = picked_model_id;
                }
                BlockType::TintedGlass => {
                    self.ui.hotbar.hotbar_tint_indices[idx] = picked_tint;
                    self.ui.hotbar.hotbar_model_ids[idx] = 0;
                    self.ui.hotbar.hotbar_paint_textures[idx] = 0;
                }
                BlockType::Painted => {
                    self.ui.hotbar.hotbar_tint_indices[idx] = picked_tint;
                    self.ui.hotbar.hotbar_paint_textures[idx] = picked_paint_texture;
                    self.ui.hotbar.hotbar_model_ids[idx] = 0;
                }
                _ => {
                    self.ui.hotbar.hotbar_model_ids[idx] = 0;
                    self.ui.hotbar.hotbar_tint_indices[idx] = 0;
                    self.ui.hotbar.hotbar_paint_textures[idx] = 0;
                }
            }
        }
    ...
    pub fn handle_global_shortcuts(&mut self) {
        ...
        if self.input.key_pressed(KeyCode::KeyI) {
            ...
            log::debug!(
                "Picture browser: {}",
                if self.ui.picture_ui.browser_open {
                    "OPEN"
                } else {
                    "CLOSED"
                }
        ...
        if self.input.key_pressed(KeyCode::Backslash) {
            ...
            log::debug!("Stencil mode: {:?}", self.ui.stencil_manager.render_mode);
        }
        ...
        if self.input.key_pressed(KeyCode::KeyR) {
            if let Some(ref mut placement) = self.ui.active_placement {
                placement.rotate_90();
                log::debug!("Rotated template to {}°", placement.rotation * 90);
            }
            // Rotate stencil placement (R key)
            if let Some(ref mut placement) = self.ui.active_stencil_placement {
                placement.rotate_90();
                log::debug!("Rotated stencil to {}°", placement.rotation * 90);
            }
        }
        ...
        if self.input.key_pressed(KeyCode::KeyP)
            && let Some(hit) = self.ui.placement.current_hit
            && let Some(BlockType::Painted) = self.sim.world.get_block(hit.block_pos)
        {
            let tex = self.ui.hotbar.hotbar_paint_textures[self.ui.hotbar.hotbar_index];
            let tint = self.ui.hotbar.hotbar_tint_indices[self.ui.hotbar.hotbar_index];
        ...
        if self.input.key_pressed(KeyCode::BracketRight)
            && self.ui.hotbar.hotbar_blocks[self.ui.hotbar.hotbar_index] == BlockType::Painted
        {
            let current = self.ui.hotbar.hotbar_paint_textures[self.ui.hotbar.hotbar_index];
            let tex = self.next_paint_texture(current, 1);
            self.ui.hotbar.hotbar_paint_textures[self.ui.hotbar.hotbar_index] = tex;
            log::debug!("Paint texture -> {}", tex);
        }
        if self.input.key_pressed(KeyCode::BracketLeft)
            && self.ui.hotbar.hotbar_blocks[self.ui.hotbar.hotbar_index] == BlockType::Painted
        {
        ...
        {
            self.ui.hotbar.hotbar_tint_indices[self.ui.hotbar.hotbar_index] =
                (self.ui.hotbar.hotbar_tint_indices[self.ui.hotbar.hotbar_index] + 1) & 0x1F;
            log::debug!(
                "Tint -> {}",
                self.ui.hotbar.hotbar_tint_indices[self.ui.hotbar.hotbar_index]
            );
        ...
        if self.ui.palette_ui.palette_open {
            let ds = self.input.scroll_diff();
            if ds.1.abs() > 0.1 {
                let len = self.ui.hotbar.hotbar_blocks.len();
                self.ui.hotbar.hotbar_index = if ds.1 > 0.0 {
                    (self.ui.hotbar.hotbar_index + len - 1) % len
                } else {
                    (self.ui.hotbar.hotbar_index + 1) % len
                };
            }
        }
    }
    ...
    fn toggle_template_browser(&mut self) {
        ...
        if self.ui.template_ui.browser_open {
            ...
            log::debug!("Template browser: OFF");
        }
    }
    ...
    fn toggle_multiplayer_panel(&mut self) {
        ...
        if self.ui.multiplayer_panel.open {
            ...
            self.input.focused = false;
            self.input.pending_grab = Some(false);
            log::debug!("Multiplayer panel: ON");
        } else {
            ...
            log::debug!("Multiplayer panel: OFF");
        }
    }
}
