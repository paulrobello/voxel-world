...
use crate::ui::{
    CameraInputs, FluidStats, HUDRenderer, HotbarInputs, HudInputs, MinimapInputs,
    MultiplayerAction, MultiplayerInputs, PaletteInputs, SettingsAction, ShapeToolInputs,
    ToolAction,
};
...
pub fn render_hud(
    ...
    let (scale_changed, editor_action, tool_action, multiplayer_action, settings_action) =
        HUDRenderer.render(
            &mut rcx.gui,
            HudInputs {
                fps: ui.frame.fps,
    ...
    // UI-002: apply settings-panel game-state mutations routed through the
    // command layer (the panel emits actions instead of mutating World/Player
    // directly inside its draw closure).
    match settings_action {
        SettingsAction::None => {}
        SettingsAction::SetFov(fov) => sim.player.camera.fov = fov,
        SettingsAction::SetAutoJump(v) => sim.player.auto_jump = v,
        SettingsAction::SetPlayerLight(v) => sim.player.light_enabled = v,
        SettingsAction::SetTimeOfDay(t) => sim.time_of_day = t,
        SettingsAction::SetMaxBlockUpdates(n) => sim.block_updates.max_per_frame = n,
        SettingsAction::ClearMinimapCache => sim.world.clear_minimap_cache(),
    }
