...
impl ApplicationHandler for App {
    ...
    fn window_event(
        ...
        if event == WindowEvent::RedrawRequested {
            match self.render(event_loop) {
                Ok(()) => {}
                Err(GpuError::SwapchainOutOfDate) => {
                    // Swapchain recreation is handled inside render(); nothing to do here.
                }
                Err(GpuError::DeviceLost(msg)) => {
                    log::warn!("[GPU] Device lost: {msg}");
                    log::warn!("[GPU] Saving world before exit...");
                    self.sim.save_all(
                        &self.ui.placement.measurement_markers,
                        &self.ui.stencil_manager,
                    );
                    self.save_preferences();
                    log::warn!("[GPU] World saved. Exiting.");
                    std::process::exit(1);
                }
                Err(GpuError::Other(msg)) => {
                    log::warn!("[GPU] Render error: {msg}");
                    // Non-fatal: log and continue; next frame may recover.
                }
            }
        }
    }
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if let Some(grab) = self.input.pending_grab.take() {
            if grab {
                ...
                log::debug!("Cursor grabbed and hidden (native macOS API)");
            } else {
                crate::macos_cursor::release_and_show();
                log::debug!("Cursor released and shown (native macOS API)");
            }
        }
