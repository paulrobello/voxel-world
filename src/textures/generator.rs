...
impl TextureLibrary {
    ...
    pub fn load() -> Self {
        ...
        if path.exists()
            ...
            && let Ok(mut lib) = serde_json::from_str::<Self>(&data)
        {
            ...
            for tex in &mut lib.textures {
                if tex.is_raw {
                    ...
                    if let Err(e) = tex.load_raw_pixels(&data_dir) {
                        log::warn!("Warning: Failed to load raw texture '{}': {}", tex.name, e);
                        // Create empty white pixels as fallback
                        tex.pixels = vec![255u8; (TEXTURE_SIZE * TEXTURE_SIZE * 4) as usize];
                    }
                } else {
