...
/// Format: For col N (0-4, left to right), bit (4-N) is set. So bit 4 = col 0 (leftmost), bit 0 = col 4 (rightmost).
const FONT_5X7: &[u8] = &[
    ...
    // ? (63) - question mark
    0x1F, 0x10, 0x02, 0x04, 0x08, 0x00, 0x01,
    // $ (36) - dollar sign
    ...
    // J (74)
    0x02, 0x02, 0x02, 0x02, 0x05, 0x05, 0x0E,
    // K (75)
...
fn get_char_index(ch: char) -> Option<usize> {
    let idx = match ch {
        ...
        '-' => 7,
        '$' => 8,
        '0'..='9' => 9 + (ch as usize) - ('0' as usize),
        'A'..='Z' => 19 + (ch as usize) - ('A' as usize),
        _ => return None,
    };
    Some(idx)
}
