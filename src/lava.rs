...
impl LavaGrid {
        ...
        Lva: Fn(Vector3<i32>) -> bool,
    {
        ...
        changed_positions.sort_by_key(|a| (a.x, a.y, a.z));
        changed_positions.dedup();
        water_contacts.sort_by_key(|a| (a.x, a.y, a.z));
    ...
    pub fn process_simulation(
        ...
        for pos in dirty_to_check {
            if pos.y >= 0
                ...
                && let std::collections::hash_map::Entry::Vacant(e) = self.cells.entry(pos)
                && let Some(BlockType::Lava) = world.get_block(pos)
            {
