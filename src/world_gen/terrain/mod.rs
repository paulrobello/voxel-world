...
pub struct ColumnData {
    ...
    pub hash: i32,
    /// Maximum absolute height difference between this column and its four
    /// cardinal neighbors at distance 4 blocks. Used by tree placement to
    /// reject steep terrain without redoing boundary-blended height samples
    /// at every tree candidate.
    ///
    /// `0.0` indicates "slope unknown / not computed" (cache may skip this
    /// for columns that will never be sampled for trees).
    pub slope: f32,
}
...
impl TerrainGenerator {
    ...
    fn get_base_height(&self, world_x: i32, world_z: i32) -> i32 {
        ...
        // Get biome + climate in a single pass (avoids duplicate climate lookup).
        let info = self.get_biome_info(world_x, world_z);
        let center_biome = info.biome;
        let climate = info.climate;
    ...
    pub fn get_height(&self, world_x: i32, world_z: i32) -> i32 {
        ...
        // Get biome + climate in a single pass (avoids duplicate climate lookup).
        let center_info = self.get_biome_info(world_x, world_z);
        let center_biome = center_info.biome;
        let climate = center_info.climate;
    ...
    pub fn get_column_data(&self, world_x: i32, world_z: i32) -> ColumnData {
        ...
        ColumnData {
            ...
            river_water_level,
            hash,
            // Slope is filled in by `ColumnDataCache` only for the columns
            // where tree placement actually reads it (step-by-4 sample grid),
            // so that non-tree callers don't pay the cost of four extra
            // boundary-blended height lookups per column.
            slope: 0.0,
        }
    }
