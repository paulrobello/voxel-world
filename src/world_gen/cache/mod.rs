...
impl ColumnDataCache {
    ...
    pub fn for_chunk(terrain: &TerrainGenerator, chunk_world_x: i32, chunk_world_z: i32) -> Self {
        ...
        // Second pass: precompute terrain slope for every column where a tree
        // placement check might read it. Trees sample columns at world coords
        // `(chunk_world_x + lx, chunk_world_z + lz)` with `lx, lz` multiples of
        // 4 inside the chunk (0..32), so we fill slope for those positions only.
        //
        // Slope is defined as the maximum absolute height difference between
        // the column and its four cardinal neighbors at distance 4 blocks —
        // matching the legacy per-tree check in `world_gen::trees`.
        for lz in (0..CHUNK_SIZE).step_by(4) {
            for lx in (0..CHUNK_SIZE).step_by(4) {
                ...
                let h = data[idx].height;
                let h_n = terrain.get_height(world_x, world_z + 4);
                let h_s = terrain.get_height(world_x, world_z - 4);
                let h_e = terrain.get_height(world_x + 4, world_z);
                let h_w = terrain.get_height(world_x - 4, world_z);
                let max_diff = (h_n - h)
                    .abs()
                    .max((h_s - h).abs())
                    .max((h_e - h).abs())
                    .max((h_w - h).abs());
                data[idx].slope = max_diff as f32;
            }
        }
