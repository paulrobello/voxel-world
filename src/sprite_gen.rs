...
pub fn run(_args: &Args, event_loop: &EventLoop<()>) -> Result<(), Box<dyn Error>> {
    ...
    log::debug!("[sprites] output: {}", out_dir.display());
    log::debug!("[sprites] placeholder: {}", missing_path.display());
    ...
    let texture_set = load_texture_atlases(
        memory_allocator.clone(),
        command_buffer_allocator.clone(),
        descriptor_set_allocator.clone(),
        &render_pipeline,
    ...
    let particle_set = get_particle_and_falling_block_set(
        memory_allocator.clone(),
        descriptor_set_allocator.clone(),
        &render_pipeline,
    )
    ...
    match model_registry.load_library_models(library_path) {
        Ok(count) if count > 0 => {
            log::debug!("[sprites] Loaded {} custom models from library", count);
        }
        Err(e) => {
            log::warn!("[sprites] Warning: Failed to load library models: {}", e);
        }
        _ => {}
    }
    ...
    let model_metadata_image = bm.model_metadata;
    ...
    {
        let mut masks = bm.brick_mask_buffer.write().unwrap();
        masks.fill(u32::MAX);
    }
    ...
    // Generate crystal sprites for each tint color
    for tint_idx in CRYSTAL_INDICES {
        render_icon(
            IconTarget::Crystal(tint_idx),
            &vk.queue,
            &render_pipeline,
            &render_image,
            &render_set,
...
enum IconTarget {
    ...
    Crystal(u8),     // tint_index for colored crystals
    Model(u8),
}
...
fn render_icon(
    ...
    let block_type = match target {
        ...
        IconTarget::Crystal(_) => BlockType::Crystal,
    ...
    let tint_index = match target {
        IconTarget::TintedGlass(idx) | IconTarget::Crystal(idx) => Some(idx),
        _ => None,
    };
    ...
    upload_chunks_batched(
        ...
        ChunkUploadConfig {
            ...
            graphics_queue_family: queue.queue_family_index(), // Same family, no ownership transfer needed
            separate_transfer_queue: false,
            voxel_image,
            model_metadata_image,
            block_custom_data_image: block_custom_data,
            texture_origin: Vector3::zeros(),
    ...
    // Render
    {
        ...
        builder
            ...
            .unwrap();
        // SAFETY: The compute pipeline and all descriptor sets (texture, brick/model,
        // metadata, distance, light, particle) were bound in the statements immediately
        // preceding this call.  The workgroup grid is derived from the icon render extent
        // and is within hardware limits.  All Vulkan resources remain valid for the
        // duration of this single-use command buffer.
        unsafe {
            builder.dispatch([
    ...
    let filename = match target {
        ...
        IconTarget::Crystal(idx) => format!("block_crystal_{}.png", idx),
