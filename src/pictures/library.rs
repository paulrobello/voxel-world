...
impl PictureLibrary {
    ...
    pub fn load() -> Self {
        ...
        match Self::load_from_file(&path) {
            Ok(library) => library,
            Err(e) => {
                log::warn!("[PictureLibrary] Failed to load: {e}");
                Self::new()
            }
        }
    }
    ...
    fn load_from_file(path: &PathBuf) -> Result<Self, String> {
        ...
        let library: Self = postcard::from_bytes(&decompressed)
    ...
    pub fn save(&self) -> Result<(), String> {
        ...
        let binary = postcard::to_stdvec(self).map_err(|e| format!("Serialization failed: {e}"))?;
