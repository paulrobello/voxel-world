...
fn main() {
    ...
    env_logger::Builder::from_env(
        env_logger::Env::default().default_filter_or("warn,renet=warn,renet_netcode=warn"),
    )
    .init();
    ...
    if app.ui.profile.auto_profile_enabled {
        log::debug!("[AUTO-PROFILE] Starting automated feature profiling");
        log::debug!(
            "[AUTO-PROFILE] Sequence: 5s baseline → for each feature: 5s OFF, 5s ON → exit"
        );
        log::debug!("[AUTO-PROFILE] Features: AO, Shadows, ModelShadows, PointLights, Minimap");
        log::debug!("[AUTO-PROFILE] Total duration: ~55 seconds");
    }
