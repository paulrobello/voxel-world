...
pub struct HudInputs<'a> {
    ...
    /// Chat history for overlay display.
        ...
        gui.immediate_ui(|gui| {
            ...
            if let Some(stencil) = console_action.stencil_loaded {
                ...
                match stencil.to_bytes() {
                    ...
                    Err(e) => {
                        log::warn!("[Console] Failed to serialize stencil for broadcast: {}", e);
                    }
                }
            }
            ...
            // Chat overlay (only in multiplayer mode)
            if game_mode != GameMode::SinglePlayer {
                MultiplayerUI::draw_chat_overlay(&ctx, chat_history, chat_display_timer);
            }
