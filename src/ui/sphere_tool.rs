//! Sphere placement tool UI.
//!
//! Provides a settings window for configuring sphere placement parameters
//! including radius, hollow mode, and placement mode.

use egui_winit_vulkano::egui;

use crate::shape_tools::{PlacementMode, SphereToolState};

/// UI for the sphere placement tool.
pub struct SphereToolUI;

impl SphereToolUI {
    /// Draw the sphere tool settings window.
    ///
    /// Placement happens via right-click, not through UI buttons.
    pub fn draw(ctx: &egui::Context, state: &mut SphereToolState) {
        if !state.active {
            return;
        }

        egui::Window::new("Sphere Tool")
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.selectable_value(&mut state.placement_mode, PlacementMode::Center, "Center")
                        .on_hover_text("Sphere center at target position");
                    ui.selectable_value(&mut state.placement_mode, PlacementMode::Base, "Base")
                        .on_hover_text("Sphere bottom rests on target surface");
                });
                ...
                // Instructions
                ui.label("Right-click to place sphere");
