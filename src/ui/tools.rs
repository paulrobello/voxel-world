...
impl ActiveTool {
    ...
    pub fn icon(&self) -> &'static str {
        match self {
            ...
            ActiveTool::Mirror => "↔",       // Left-right arrow
...
impl ToolsPaletteUI {
    ...
    fn draw_settings_panel(
        ...
        match state.active_tool {
            ...
            // Every remaining tool renders its controls in a dedicated window
            // (see the per-tool *ToolUI::draw calls in ui/mod.rs); the palette's
            // embedded settings panel only hosts Measurement / Stencil / FloodFill.
            _ => {
                ui.label(
                    egui::RichText::new(format!(
