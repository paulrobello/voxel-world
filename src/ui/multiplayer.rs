...
// Re-export ChatEntry for use in other modules
pub use crate::app_state::multiplayer::ChatEntry;
...
impl MultiplayerUI {
    ...
    pub fn draw(
        ...
        log::debug!("[MultiplayerUI] Panel is open, tab: {:?}", state.tab);
        ...
        {
            log::debug!("[MultiplayerUI] Returning action: {:?}", action);
        }
        ...
        if host.is_hosting {
            ...
            ui.vertical(|ui| {
                ...
                if ui
                    ...
                    .clicked()
                    && let Ok(port) = host.port.parse::<u16>()
                {
    ...
    /// Draws the chat message overlay (bottom-left corner).
    /// Shows recent chat messages with fade effect.
    pub fn draw_chat_overlay(
        ctx: &egui::Context,
        chat_history: &[ChatEntry],
        display_timer: Option<f32>,
    ) {
        // Don't draw if timer is None or <= 0
        if display_timer.is_none() || display_timer.unwrap_or(0.0) <= 0.0 {
            return;
        }

        // Don't draw if no messages
            ...
            .show(ctx, |ui| {
                egui::Frame::new()
                    ...
                    .show(ui, |ui| {
                        ...
                        let start = chat_history.len().saturating_sub(10);
