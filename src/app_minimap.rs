...
pub fn prepare_minimap_image(
    ...
    let moved = (current_pos.x - ui.minimap_last_pos.x).abs() >= 1
        || (current_pos.z - ui.minimap_last_pos.z).abs() >= 1;
    // Check if player rotated significantly (5 degrees) - only matters when rotate mode is on
    let yaw_changed = ui.minimap.rotate && (camera_yaw - ui.minimap_last_yaw).abs() > 0.087; // ~5 degrees
    // Check if enough time has passed (0.1 seconds for rotation, 0.5 for position)
    let time_elapsed = ui.minimap_last_update.elapsed().as_secs_f32();
    let time_ok = if ui.minimap.rotate {
        time_elapsed >= 0.1
    } else {
        time_elapsed >= 0.5
    };

    if ((moved || yaw_changed) && time_ok) || ui.minimap_cached_image.is_none() {
