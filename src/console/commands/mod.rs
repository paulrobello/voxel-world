...
pub fn help() -> CommandResult {
  ...
  name <your_name>, nickname, setname
    Set your player display name (multiplayer only).
    Name must be 1-32 characters (letters, numbers, underscores, spaces).
    Example: name Builder123

  chat <message>, say, msg
    Send a chat message to all players (multiplayer only).
