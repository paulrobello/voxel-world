...
pub fn template(
    ...
    match subcommand {
        "save" => template_save(
            args, selection, world, water_grid, library, confirmed, author,
        ),
...
fn template_save(
    ...
    let template = match VxtFile::from_world_region(
        world,
        water_grid,
        name.clone(),
        author.to_string(),
    ...
    match library.save_template(&template) {
        Ok(_) => {
            ...
            if let Err(e) = crate::templates::rasterizer::generate_template_thumbnail(
                ...
                log::warn!("[Template] Warning: Failed to generate thumbnail: {}", e);
