...
impl LanDiscovery {
    ...
    pub fn update(&mut self) {
        ...
        if should_request && let Err(e) = self.send_discovery_request() {
            log::error!("[Discovery] Failed to send discovery request: {}", e);
        }

        // Receive pending packets. Cap per call so a broadcast storm (DHCP,
        // mDNS, etc. on the same UDP port) can't stall the game loop by
        // forcing this function to drain the full socket queue synchronously.
        const MAX_PACKETS_PER_UPDATE: usize = 16;
        let mut buf = [0u8; MAX_PACKET_SIZE];
        let mut remaining = MAX_PACKETS_PER_UPDATE;
        while remaining > 0 {
            match self.socket.recv_from(&mut buf) {
                Ok((len, addr)) => {
                    log::debug!("[Discovery] Received {} bytes from {}", len, addr);
                    if let Some(server) = Self::parse_announcement(&buf[..len], addr) {
                        ...
                        log::debug!("[Discovery] Failed to parse announcement");
                    }
                }
        ...
        if remaining == 0 {
            log::debug!(
                "[Discovery] Hit per-update packet cap ({}); deferring remainder",
    ...
    fn parse_announcement(data: &[u8], addr: SocketAddr) -> Option<DiscoveredServer> {
        // Enforce strict packet length bounds before any deserialization.
        // Too-short: no room for header + payload.
        // Too-long: reject oversized packets that could exploit the deserializer.
        const MIN_PACKET_LEN: usize = 5; // 4 magic + 1 type byte
        if data.len() < MIN_PACKET_LEN || data.len() > MAX_PACKET_SIZE {
            return None;
        }
...
impl DiscoveryResponder {
    ...
    ///
    /// Binds the fixed `DISCOVERY_PORT` with `SO_REUSEADDR` set so that a
    /// quick host-restart (crash + relaunch) doesn't fail because the old
    /// socket is still in `TIME_WAIT` on the same port.
    pub fn new(server_name: String, game_port: u16, max_players: u8) -> io::Result<Self> {
        use socket2::{Domain, Protocol, Socket, Type};
        use std::net::{Ipv4Addr, SocketAddrV4};

        let addr: SocketAddrV4 = SocketAddrV4::new(Ipv4Addr::UNSPECIFIED, DISCOVERY_PORT);
    ...
    pub fn update(&self, player_count: u8) {
        // Same per-call packet cap as the client-side LanDiscovery loop so a
        // broadcast storm can't force this responder to empty the socket
        // queue synchronously every tick.
        const MAX_PACKETS_PER_UPDATE: usize = 16;
        let mut buf = [0u8; MAX_PACKET_SIZE];
        let mut remaining = MAX_PACKETS_PER_UPDATE;
            ...
            match self.socket.recv_from(&mut buf) {
                Ok((len, addr)) => {
                    if self.is_discovery_request(&buf[..len]) {
                        log::debug!("[Discovery] Received discovery request from {}", addr);
                        if let Err(e) = self.send_announcement(addr, player_count) {
                            log::error!("[Discovery] Failed to send announcement: {}", e);
                        } else {
                            log::debug!("[Discovery] Sent announcement to {}", addr);
                        }
                    }
                }
                Err(ref e) if e.kind() == ErrorKind::WouldBlock => {
...
mod tests {
    ...
    /// L3 regression guard: SO_REUSEADDR (+ SO_REUSEPORT on unix) must allow
    /// a second bind on the same discovery port, otherwise a host restart
    /// after a crash fails until the kernel releases the port.
    #[test]
    fn test_discovery_responder_allows_reuseaddr_rebind() {
        // First bind holds the port.
        let first = DiscoveryResponder::new("first".into(), 5000, 4);
        assert!(first.is_ok(), "first bind must succeed: {:?}", first.err());

        // Second bind on the same fixed port must also succeed thanks to
        // SO_REUSEADDR. Without the fix this returns EADDRINUSE.
        let second = DiscoveryResponder::new("second".into(), 5000, 4);
        assert!(
            second.is_ok(),
            "second bind on DISCOVERY_PORT must succeed via SO_REUSEADDR, got: {:?}",
            second.err()
        );
    }
