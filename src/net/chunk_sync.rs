...
impl ChunkSyncManager {
    ...
    pub fn request_chunks_around(
        ...
        needed.sort_by_key(|b| std::cmp::Reverse(b.1));
    ...
    fn recalculate_priorities(&mut self) {
        // Extract fields used by priority calculation to avoid borrow conflict
        let player_pos = self.last_player_pos;
        let look_dir = self.last_look_dir;

        for request in self.pending_requests.values_mut() {
            let player_chunk = world_to_chunk(player_pos);
            let dx = (request.position[0] - player_chunk[0]).abs();
            let dy = (request.position[1] - player_chunk[1]).abs();
            let dz = (request.position[2] - player_chunk[2]).abs();
            let distance = dx + dy + dz;

            request.priority = if distance == 0 {
                ChunkPriority::PlayerPosition
            } else {
                let to_chunk = chunk_center(request.position, player_pos);
                let dot = dot_product(to_chunk, look_dir);
                if dot > 0.7 && distance <= 4 {
                    ChunkPriority::ViewDirection
                } else if distance <= 2 {
                    ChunkPriority::Adjacent
                } else {
                    ChunkPriority::Background
                }
            };
    ...
    ///
    /// 0.85 ≈ 32° is coarse enough that normal mouselook doesn't trigger a
    /// full priority recalc every frame, but fine enough that a deliberate
    /// 90° turn still updates priorities promptly.
    ...
    ///
    /// Half-chunk threshold: a player walking at 5 blocks/s hits this once
    /// every ~3 seconds, so the 729-chunk recalc in `recalculate_priorities`
    /// stays cheap on average even at 20 Hz input updates.
...
impl SerializedChunk {
    ...
    pub fn compress(&self) -> Result<Vec<u8>, &'static str> {
        ...
        for (idx, data) in &self.model_data {
            raw.extend_from_slice(&(*idx as u16).to_le_bytes());
    ...
    pub fn decompress(compressed_data: &[u8]) -> Result<Self, &'static str> {
        ...
        for _ in 0..model_count {
            let idx = read_u16(&raw, &mut cursor)? as usize;
...
mod tests {
    ...
    /// Regression guard for H12 (chunk-vs-block-delta ordering).
    ///
    /// Simulates two inputs for the same position arriving in opposite orders
    /// over the wire: (ChunkData carrying block X, BlockChanged → Y) versus
    /// (BlockChanged → Y, ChunkData carrying X). After each ordering the
    /// "last applied wins" rule on the client must leave the block at Y —
    /// the value from BlockChanged.
    ///
    /// If the ChunkStream channel regresses back to Unreliable or a chunk
    /// handler starts stomping later block deltas, this test flips.
    #[test]
    fn test_chunk_and_block_delta_converge_regardless_of_order() {
        use crate::chunk::{BlockType, Chunk};
        use crate::net::protocol::{BlockChanged, BlockData, ChunkData};

        // Build a serialized chunk that has Stone at local [0, 0, 0].
        let mut chunk = Chunk::new();
        chunk.set_block(0, 0, 0, BlockType::Stone);
        let serialized = SerializedChunk::from_chunk([0, 0, 0], &chunk);
        let compressed = serialized.compress().expect("compress");
        let chunk_msg = ChunkData {
            position: [0, 0, 0],
            version: 1,
            compressed_data: compressed,
        };
        let block_msg = BlockChanged {
            position: [0, 0, 0],
            block: BlockData::from(BlockType::Sand),
        };

        // Order A: chunk first, then block delta. Block delta wins.
        let applied_a = apply_to_client(&chunk_msg, &block_msg, true);
        ...
        let applied_b = apply_to_client(&chunk_msg, &block_msg, false);
        assert_eq!(
            applied_b,
            BlockType::Sand,
            "BlockChanged delta must win even when ChunkData arrives later"
        );
    }

    /// Helper that simulates the client's apply path: returns the final block
    /// type at [0, 0, 0] after applying chunk + block in the given order.
    /// `chunk_first=true` means ChunkData arrives before BlockChanged.
    fn apply_to_client(
        chunk_msg: &crate::net::protocol::ChunkData,
        block_msg: &crate::net::protocol::BlockChanged,
        chunk_first: bool,
    ) -> crate::chunk::BlockType {
        let mut local_chunk = crate::chunk::Chunk::new();

        let apply_chunk = |c: &mut crate::chunk::Chunk| {
            let sc = SerializedChunk::decompress(&chunk_msg.compressed_data).unwrap();
            let rebuilt = sc.to_chunk().unwrap();
            *c = rebuilt;
        };
        let apply_block = |c: &mut crate::chunk::Chunk| {
            // Positions in the test are all at local (0,0,0); no wrapping
            // conversion needed.
            c.set_block(
                block_msg.position[0] as usize,
                block_msg.position[1] as usize,
                block_msg.position[2] as usize,
                block_msg.block.block_type,
            );
        };

        if chunk_first {
            apply_chunk(&mut local_chunk);
            apply_block(&mut local_chunk);
        } else {
            apply_block(&mut local_chunk);
            apply_chunk(&mut local_chunk);
            apply_block(&mut local_chunk);
