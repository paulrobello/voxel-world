...
impl DayCycleSync {
    /// Creates a new day cycle sync tracker.
    pub fn new() -> Self {
        Self {
            paused: false,
            time_of_day: 0.25, // Default to morning
            stats: DayCycleSyncStats::default(),
        }
    }

    /// Updates the pause state and returns a message if changed.
    ///
    /// # Arguments
    ...
    ///
    /// # Returns
    /// `true` if the state changed, `false` if unchanged.
    pub fn set_paused(&mut self, paused: bool, time_of_day: f32) -> bool {
        if self.paused != paused || (self.time_of_day - time_of_day).abs() > 0.001 {
            self.paused = paused;
