...
- **Multiplayer Networking**: Host/client model with block, water, and entity sync

### Key Files
- `main.rs` - Vulkan setup, render loop, input, physics, HUD
- `chunk.rs` - BlockType enum, chunk storage (32³), metadata
- `world/` - Multi-chunk management, terrain generation, biomes, climate, rivers, trees
- `world_streaming.rs` - Chunk streaming, origin shifts, GPU upload pipeline
- `chunk_loader.rs` - Background thread pool for async chunk generation
- `gpu_resources.rs` - Vulkan buffer management, batched chunk uploads
- `svt.rs` - Sparse Voxel Tree (64-bit brick mask + distance field)
- `sub_voxel/` - Sub-voxel model system (8³/16³/32³), registry, palettes
- `block_interaction/` - Block placement/breaking, hotbar, palette UI
- `water.rs` / `lava.rs` - Fluid simulation (cellular automata)
- `net/` - Multiplayer networking (host/client, sync protocols)
- `ui/` - HUD, tools palette, template browser, stencil browser, settings
...
- `shaders/common.glsl` - Push constants, buffer layouts
- `shaders/materials.glsl` - Texture sampling, emission, atlas mapping
- `shaders/lighting.glsl` - Point lights, shadows, AO
- `shaders/models.glsl` - Sub-voxel model ray marching
- `shaders/overlays.glsl` - Selection markers, measurement blocks, stencils
...
- Climate parameters: Temperature, Humidity, Continentalness, Erosion, Weirdness (5-noise system)
- 15 Surface Biomes: Ocean, Beach, Plains, Forest, DarkForest, BirchForest, Taiga, SnowyPlains, SnowyTaiga, Desert, Savanna, Swamp, Mountains, Meadow, Jungle
- 3 Underground Biomes: LushCaves, DripstoneCaves, DeepDark
- River generation system
- Biome-specific height curves and surface/subsurface blocks
- 8 tree types: Oak, Pine, Willow, Cactus, Birch, Jungle, Acacia, Snow
- Ground cover: Tall Grass, Flowers, Mushrooms, Lily Pads, Ferns, Seagrass
...
- [x] Cube: square icon (🟦)
- [x] Bridge: pin icon (📍)
- [x] Cylinder, Wall, Floor, Circle, Stairs, Arch (Phase 17)
- [x] Cone/Pyramid, Replace, Mirror, Clone/Array (Phase 17)
- [x] Torus, Helix, Polygon, Bezier, Pattern Fill, Scatter, Hollow, Terrain Brush (Phase 18)
- [x] Active tool: highlighted border with green indicator dot
- **Total: 24 tools** plus None state
...
- **Status**: COMPLETE (previously deferred, now implemented)
- **Scope**: 1-4 players, cooperative building
- **Free-for-all**: No permissions or build zones
- **Sync Requirements**: Blocks, water, templates, tool visualizations, tree falls, falling blocks, day cycle
- **Architecture**: Host/client model (`src/net/`), server thread, block sync, water sync
- **CLI**: `--multiplayer`, `--host`, `--connect`, `--port` options
- **Makefile**: `run-host`, `run-client`, `reset-host`, `reset-client` targets
...
- **Water simulation**: CPU-bound, could move to GPU compute shader
...
make run ARGS="--world-gen flat"    # Flat world generation (-g)
...
make run ARGS="--quality ultra"     # Graphics quality preset (-q: potato, low, medium, high, ultra)
make run ARGS="--multiplayer --host --port 12345"  # Host multiplayer game
make run ARGS="--multiplayer --connect <ip> --port 12345"  # Join multiplayer game
```
...
- **Phase 8: Multiplayer Networking** (2026-01 to 2026-04): ✅ COMPLETE
  - **Host/Client Architecture**: `src/net/` module with server thread, sync protocols
  - **Block Sync**: Real-time block placement/breaking between players
  - **Water/Lava Sync**: Fluid simulation state synchronized across clients
  - **Entity Sync**: Falling blocks, tree falls, player positions
  - **Day Cycle Sync**: Time-of-day synchronized across all clients
  - **CLI Options**: `--multiplayer`, `--host`, `--connect`, `--port`
  - **Makefile Targets**: `run-host`, `run-client`, `reset-host`, `reset-client`

- **Biome System Expansion** (2026-01 to 2026-04): ✅ COMPLETE
  - Expanded from 5 to 18 biomes (15 surface + 3 underground)
  - 5-parameter climate system (Temperature, Humidity, Continentalness, Erosion, Weirdness)
  - 8 tree types: Oak, Pine, Willow, Cactus, Birch, Jungle, Acacia, Snow
  - River generation system
  - `src/world_gen/` restructured into biome, caves, climate, rivers, terrain, trees, vegetation submodules

- **Performance Optimization Pass** (2026-04): ✅ COMPLETE
  - Origin shift stall reduced ~47% via zero-slice skip
  - Z-adjacent chunks merged into single copy regions during shifts
  - Workgroup-shared chunk_flags cache in traverse.comp
  - Sparsified SubVoxelModel palette_emission
  - Pre-computed lava/mushroom pulse on CPU
  - Delta-skip PlayerInput sends when idle
  - Unified per-block metadata into single enum-keyed map
  - Shared palette atlas for sub-voxel models
  - Incremental model atlas upload
  - Async ring-buffer for atlas texture/picture uploads
  - Terrain generation optimizations (climate consolidation, tree slope cache)
...
*Plan Version: 3.8 - Synced with codebase: multiplayer, 18 biomes, 24 tools, performance pass*
