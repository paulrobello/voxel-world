// Sub-voxel model ray marching -- resolution-agnostic implementation.
// Supports three atlas tiers (8³, 16³, 32³); the same DDA traversal logic
// is used regardless of resolution, with the atlas sampled via normalized coords.
// Model layout: 16 models per row, 16 rows = 256 models max per tier.
// Atlas tier is selected automatically based on the model's registered resolution.
...
// Extract palette_id from model flags (bits 16-23; see registry.rs::pack_model_flags).
uint getModelPaletteId(uint model_id) {
    return (model_properties[model_id].flags >> 16u) & 0xFFu;
}

// Get model color from the shared palette atlas.
// model_id: 0-255 (translated to palette_id via model_properties)
// palette_idx: 0-31 (palette index within the palette, 32 colors per palette)
vec4 getModelPaletteColor(uint model_id, uint palette_idx) {
    uint palette_id = getModelPaletteId(model_id);
    vec2 uv = vec2(
        (float(palette_id) + 0.5) / 256.0,
        (float(palette_idx) + 0.5) / 32.0
    );
    return texture(modelPalettes, uv);
}

// Lightweight alpha-only palette lookup for shadow rays where only opacity matters
float getModelPaletteAlpha(uint model_id, uint palette_idx) {
    return getModelPaletteColor(model_id, palette_idx).a;
}
...
bool marchSubVoxelModel(
    ...
    for (int i = 0; i < maxSteps; i++) {
        ...
        // Apply rotation and sample (fast path for unrotated models)
        ivec3 rotatedPos;
        if (rotation == 0u) {
            rotatedPos = voxel;
        } else if (isFrame) {
            rotatedPos = clamp(transformFramePos(voxel, rotation, res), ivec3(0), ivec3(int(res) - 1));
        } else {
            rotatedPos = clamp(rotateModelPos(voxel, rotation, res), ivec3(0), ivec3(int(res) - 1));
        }
...
float marchSubVoxelShadow(
    ...
    int stepsLeft = clamp(maxSteps, 1, int(res) * 2 + 8);
    ...
    for (int i = 0; i < stepsLeft; i++) {
        ...
        if (palette_idx != 0u) {
            // Get alpha from palette for opacity check (lightweight path for shadows)
            float paletteAlpha = getModelPaletteAlpha(model_id, palette_idx);
            if (paletteAlpha >= 0.99) {
            ...
            if (!isInternalFace) {
                ...
                // Fetch full color only for translucent tint accumulation
                vec3 paletteRgb = getModelPaletteColor(model_id, palette_idx).rgb;
                accumulatedTint *= mix(vec3(1.0), paletteRgb, absorbed);
